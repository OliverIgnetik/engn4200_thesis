...
In this section the results obtained by the NMF model will be discussed.
...
\begin{figure}
    ...
    \caption{Hyperparameter tuning with accuracy metrics for monophonic AMT system}
...
\autoref{results:fig:dictionary}) can be thought of as DFTs of the seperate
...
In summary the NMF model performed extremely well for monophonic AMT however is not
appropriate for polyphonic transcription. The crucial points of discovery are:

\begin{itemize}
    \item NMF model provides high accuracy metrics for monophonic music
...
\subsection{Discussion}
In this section the results obtained by the NN model will be discussed.
...
\begin{figure}
    ...
    \caption{Loss function f1 measure curves for neural network model with optimal parameters}
...
\begin{table}
    ...
    \end{tabular}
    \caption{Hyperparameter tuning for Neural Network approach on polyphonic AMT}
    \label{results:table:nn-grid}
\end{table}


\subsection{Summary}
In summary, the NN model performed well for polyphonic AMT however further work
is needed to convert the output to an interpretible form that more closely resembles the piano MIDI roll representation.
