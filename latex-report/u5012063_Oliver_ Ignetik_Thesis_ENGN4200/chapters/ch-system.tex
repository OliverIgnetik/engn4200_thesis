...
\begin{figure}
    ...
    \caption{Typical architecture of an AMT system. \cite{amtfc2013:Benetos}}
...
passed to the NMF\_model class which is the main class for performing the
experiments in this investigation. As can be seen in
\autoref{system:fig:nmf-model} the audio data is passed to the NMF class where
the constructor takes two important parameters to determine the spectrogram
\emph{hop length} and \emph{window size}. The NMF class has the following
important methods :

\begin{itemize}
    \item make\_stft - builds the spectrogram based on the hop length and window size
    \item NMF\_decomposition - performs the NMF decomposition
...
A feed forward NN will be used to transcribe music from the musicnet database.
