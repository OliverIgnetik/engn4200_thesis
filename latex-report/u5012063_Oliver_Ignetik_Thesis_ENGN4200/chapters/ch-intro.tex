...
highly correlated over both time and frequency, means that \ac{AMT} is still
considered an open problem in the literature. Usually an AMT system takes an
audio waveform as input, computes a time-frequency representation and outputs
pitches over time or ideally a typeset music score. Most approaches are designed
to achieve an intermediate goal in AMT, which does not actually resemble musical
notation as shown in \autoref{intro:fig:AMT-process}.
...
music where even the most advanced systems are far behind meeting the accuracy of
trained musicians \cite{spma2011:Klapuri}.

\begin{figure}[!b]
      \centering
      \includegraphics[width=.6\linewidth]{\Pic{jpg}{AMT-process}}
      \caption[Transcription Process]{\ac{AMT} process. AMT often involves the process
            of converting audio waveform to mid-level representations such as the piano roll representation which
            encode the pitch and timing of note onsets and offsets.
            Taken from NUS ISMIR 2019 \cite{ISMIR-tut:Benetos} }
...
Commercially available applications include Melodyne (\url{http://www.celemony.com/en/melodyne }), Transcribe!(\url{https://www.seventhstring.com/xscribe/ })  and AudioScore.
