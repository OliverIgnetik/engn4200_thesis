...
%   \label{review:fig:spectrum} \end{figure}
...
More recently, one approach proposed an algorithm for multi-pitch detection and
beat structure analysis. The NMF objective function is constrained using
...
NNs have a number of advantages over traditional machine learning algorithms.
...
\begin{figure}[!b]
  ...
  \caption{An intuitive graphical understanding of gradient descent optimization}
...
One other crucial concept in NNs and machine learning in general is the concept
...
\begin{enumerate}
  ...
  \item \emph{activation functions} - mathematical equations that determine the
        ...
        include : ReLU (Rectified Linear Unit) (Shown in
        \autoref{review:fig:activation-function}), hyperbolic tangent, softmax
        ...
        \begin{figure}[!b]
          \centering
          \includegraphics[width=.6\linewidth]{\Pic{png}{RELU}}
          \caption{ReLU activation function and Leaky ReLU used to address the
            vanishing neuron problem that occurs when neurons stop contributing
            to the model output}
          \label{review:fig:activation-function}
        \end{figure}
  ...
  \item \emph{epoch} - when an entire dataset is passed forward and backward
        through the neural network only once. Typically setting a higher number
  ...
  \item \emph{loss function} - the loss function is used in backpropagation to
        ...
        \begin{figure}[!b]
          \centering
          \includegraphics[width=.6\linewidth]{\Pic{png}{log_loss}}
